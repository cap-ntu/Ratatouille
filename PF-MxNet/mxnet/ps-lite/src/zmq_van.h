...
protected:
    ...
    int SendMsg(const Message& msg) override {
        ...
        long data_size=snappy::Compress(meta_buf,meta_size,output2);

        zmq_msg_init_data(&meta_msg,(char*)output2->c_str(), data_size, FreeData, output2);
        delete [] static_cast<char*>(meta_buf);
        while (true) {
            if (zmq_msg_send(&meta_msg, socket, tag) == data_size) break;
        ...
        for (int i = 0; i < n; ++i) {
            ...
            data_size=snappy::Compress(data->data(),data->size(), output);
            //      std::cout<<"compressed"<<std::endl;
            zmq_msg_init_data(&data_msg,(char*)output->data() ,data_size, FreeData, output);
    ...
    int RecvMsg(Message* msg) override {
        ...
        for (int i = 0; ; ++i) {
            ...
            std::string *output2=new std::string();
            snappy::Uncompress(buf,size,output2);
            ...
            if (i == 0) {
                ...
                // task
                //std::cout<<"zz\n ";
                UnpackMeta((char*)output2->c_str(),output2->size(), &(msg->meta));
                zmq_msg_close(zmsg);
                ...
                //std::string *output2=new std::string();
                //snappy::Uncompress(buf, size, output2);
                size_t t=output2->size();
                char *y=(char*)output2->c_str();
                data.reset(y, t, [zmsg,size,output2](char* buf) {
                    zmq_msg_close(zmsg);
                    delete zmsg;
                    delete output2;
                });
