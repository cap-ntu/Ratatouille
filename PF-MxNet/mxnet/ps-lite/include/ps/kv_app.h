...
class KVWorker : public SimpleApp {
 ...
 static pushStruct* searchKey(std::vector<pushStruct>* arr,Key key){
  ...
  void push(pushStructArray& j,pushStruct& i){
	  std::vector<pushStruct> *ptr;
	  if(typeid(i.vals[0]).hash_code()==typeid(char).hash_code())
		 ptr=&j.array[0];
	  else if(typeid(i.vals[0]).hash_code()==typeid(int8_t).hash_code())
	  		 ptr=&j.array[1];
	  else if(typeid(i.vals[0]).hash_code()==typeid(int16_t).hash_code())
	  		 ptr=&j.array[2];
	  else if(typeid(i.vals[0]).hash_code()==typeid(int32_t).hash_code())
	  	  		 ptr=&j.array[3];
	  else if(typeid(i.vals[0]).hash_code()==typeid(int64_t).hash_code())
	  		 ptr=&j.array[4];
	  else if(typeid(i.vals[0]).hash_code()==typeid(uint8_t).hash_code())
	  	  		 ptr=&j.array[5];
	  else if(typeid(i.vals[0]).hash_code()==typeid(uint16_t).hash_code())
	  	  		 ptr=&j.array[6];
	  else if(typeid(i.vals[0]).hash_code()==typeid(uint32_t).hash_code())
	  	  	  		 ptr=&j.array[7];
	  else if(typeid(i.vals[0]).hash_code()==typeid(uint64_t).hash_code())
	  	  		 ptr=&j.array[8];
	  else if(typeid(i.vals[0]).hash_code()==typeid(float).hash_code())
	  	  	  		 ptr=&j.array[9];
	  else if(typeid(i.vals[0]).hash_code()==typeid(double).hash_code())
	  	  		 ptr=&j.array[10];

	  if(ptr->empty())ptr->push_back(i);
	  else
	  {
		  pushStruct* structptr=searchKey(ptr,i.keys);				//assuming same key same cb
		  if(structptr!=0){
			  double sum=0;
				  for(int y=0;y<structptr->lens;y++)
				  {  structptr->vals[y]=i.vals[y]+structptr->vals[y];
				  	  sum+= structptr->vals[y];
				  }
				  structptr->average=sum/structptr->lens;
	  ...
	  std::sort(ptr->begin(),ptr->end(),sortAvg);

  }
  bool pop(pushStructArray& j,std::vector<PopMsg>& i ,double thres){
	  pushStruct temp;
	  PopMsg merged;

	  for(size_t t=0;t<11;t++)
	  {   bool add=false;
		  while(j.array[t].size()>0 && thres<(temp=j.array[t].back()).average)
		  {		add=true;
			  j.array[t].pop_back();							// pushing data to popmsg
				  merged.cb=nullptr;
				  merged.cmd=0;
				  merged.keys.push_back(temp.keys);
				  merged.lens.push_back(temp.lens);
				  for(size_t y=0;y<temp.lens;y++)
				  merged.vals.push_back(temp.vals[y]);
				//std::cout<<"Pop";
		  }
	  if(add)
	  i.push_back(merged);
...
static double calculateavg(Val* vals,size_t size){
	double sum=0;
	for(size_t y=0;y<size;y++)sum+=abs(vals[y]);
	return sum/size;}
  ...
  int Push( std::vector<Key> keys,
	  ...
	  const double init_Threshold=GetEnv("init_threshold",0.0);
	  static unsigned long iteration=0;
	  static pushStructArray array;
	  iteration++;
	  double  thres = init_Threshold / sqrt(iteration);
	  if(lens.empty())lens.assign(keys.size(),vals.size()/keys.size());
	  size_t size=keys.size();
	  //SArray<Key> newkey;
	  //SArray<Val> newvals;
	  //SArray<int> newlen;
	  std::vector<PopMsg> msg;
	  pop(array,msg,thres);
	  for(size_t u=0;u<msg.size();u++)
		  ZPush(SArray<Key>(msg[u].keys),SArray<Val>(msg[u].vals),SArray<int>(msg[u].lens),msg[u].cmd,msg[u].cb);
	//std::cout<<"test";
	  int ptr=0;		//pointing to process vals saaray
	  for(size_t s=0;s<size;s++)
	  {		pushStruct structs;
		  double avg=calculateavg(&vals[ptr],lens[s]);
		  if(avg<thres && avg>0)
		  {
			  structs.average=avg;
			  structs.cb=cb;
			  ...
			  structs.keys=keys[s];
			  keys.erase(keys.begin()+s);

			  int pt=ptr;
			  for(int y=0;y<lens[s];y++){ structs.vals.push_back(vals[pt]);pt++;}
			   pt=ptr;
			  for(int y=0;y<lens[s];y++){ vals.erase(vals.begin()+pt);}

			  structs.lens=lens[s];
			  lens.erase(lens.begin()+s);
			  push(array,structs);
			//std::cout<<"Push"<<avg<<thres<<std::endl;
		  }
		  ptr+=lens[s];
