...
public:
    ...
    void Pull(const std::vector<int>& keys,
	    ...
	    const char* env_var=getenv("INIT_THRESHOLD");
	    if(env_var!=NULL)
		init_Threshold=atof(env_var);
	   double  thres= init_Threshold / cbrt(iteration);
            real_t avg=0;
            real_t sum=0;
            if(!SyncMode && keyexist && size>0 && !isnan(data[0])) {
            ...
            RunContext rctx, Engine::CallbackOnComplete cb) {
                ...
                pskv.keys, vals, &pskv.lens, 0, [vals, cb]() {
                    delete vals;
                    cb();
                });
            };
	...
	if(cmd_id==kvstore::kSyncMode && cmd_body=="")
		SyncMode=true;
	std::cout<<SyncMode<<"\n";
    }
...
private:
    ...
    static bool sortAvg(pushStruct i,pushStruct j) {
        return i.average>j.average;   //sort dscending
    }
    ...
    void Push_(const std::vector<int>& keys,
        ...
        float init_Threshold=0;//dmlc::GetEnv("init_threshold",1.0);
	const char* env_var=getenv("INIT_THRESHOLD");
        if(env_var!=NULL)
                init_Threshold=atof(env_var);
	static std::vector<pushStruct> pushStruct_vector;
        static unsigned long iteration=1;
        double  thres= init_Threshold / cbrt(iteration);
        if(!SyncMode)pop(pushStruct_vector,thres);
        std::vector<int> uniq_keys;
        std::vector<std::vector<NDArray> > grouped_vals;
        GroupKVPairs(keys, values, &uniq_keys, &grouped_vals);
        NDArray *ptr;
